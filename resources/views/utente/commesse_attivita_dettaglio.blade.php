@include('utente.common.header')
                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item"><a href="{{ url('utente/commesse/'.$commessa->id.'/attivita') }}">Attività</a></li>
            ...
            <div class="row">
                    ...
                    <div class="card mt-4">
                        <div class="card-header bg-soft-warning d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0">
                                <i class="ri-sticky-note-line me-2"></i>Note Attività
                            </h5>
                            <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modificaNoteModal">
                                <i class="ri-edit-line me-1"></i>Modifica Note
                            </button>
                        </div>
                        <div class="card-body">
                            @if($attivita->note)
                                <div class="bg-light p-3 rounded">
                                    <p class="mb-0">{!! nl2br(e($attivita->note)) !!}</p>
                                </div>
                            @else
                                <div class="text-center p-4">
                                    <div class="avatar-md mx-auto mb-3">
                                        <div class="avatar-title bg-light text-muted rounded-circle">
                                            <i class="ri-sticky-note-line fs-24"></i>
                                        </div>
                                    </div>
                                    <h6 class="mb-2">Nessuna nota disponibile</h6>
                                    <p class="text-muted mb-0">Clicca su "Modifica Note" per aggiungere annotazioni a questa attività.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="hstack gap-2">
                                                        ...
                                                        <form action="{{ url('utente/commesse/'.$commessa->id.'/attivita/'.$attivita->id.'/dettaglio') }}" method="POST" style="display: inline;">
                                                            @csrf
                                                            <input type="hidden" name="elimina_allegato" value="1">
                                                            <input type="hidden" name="id_allegato" value="{{ $allegato->id }}">
                                                            <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Sei sicuro di voler eliminare questo allegato?')" data-bs-toggle="tooltip" title="Elimina">
                ...
                <div class="col-xl-4">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            @if(!$attivita->data_inizio_effettiva && $attivita->stato != 'completata')
                                ...
                                <form action="{{ url('utente/commesse/'.$commessa->id.'/attivita/'.$attivita->id.'/dettaglio') }}" method="POST" class="mb-3">
                                    @csrf
                                    <input type="hidden" name="start_attivita" value="1">
                            ...
                            @elseif($attivita->data_inizio_effettiva && !$attivita->data_fine_effettiva)
                                ...
                                <form action="{{ url('utente/commesse/'.$commessa->id.'/attivita/'.$attivita->id.'/dettaglio') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="fine_attivita" value="1">
                                    <div class="mb-3">
                    ...
                    @if($attivita->data_inizio_effettiva && !$attivita->data_fine_effettiva)
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <form action="{{ url('utente/commesse/'.$commessa->id.'/attivita/'.$attivita->id.'/dettaglio') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="aggiorna_attivita" value="1">
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        <label for="note_avanzamento" class="form-label">Note di avanzamento</label>
                                        <textarea class="form-control" id="note_avanzamento" name="note" rows="2" placeholder="Inserisci eventuali note di avanzamento">{{ $attivita->note }}</textarea>
                                    </div>
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            <a href="{{ url('utente/commesse/'.$commessa->id.'/attivita') }}" class="btn btn-outline-secondary w-100">
                                <i class="ri-arrow-left-line me-1 align-middle"></i> Torna alle attività
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal fade" id="modificaNoteModal" tabindex="-1" aria-labelledby="modificaNoteModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modificaNoteModalLabel">
                        <i class="ri-sticky-note-line me-2"></i>Modifica Note Attività
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ url('utente/commesse/'.$commessa->id.'/attivita/'.$attivita->id) }}" method="POST">
                    @csrf
                    <input type="hidden" name="modifica_note" value="1">
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="note_modal" class="form-label">Note</label>
                            <textarea class="form-control" id="note_modal" name="note" rows="8" placeholder="Inserisci le note per questa attività...">{{ $attivita->note }}</textarea>
                            <div class="form-text">
                                <i class="ri-information-line me-1"></i>
                                Puoi utilizzare le note per tracciare avanzamenti, problemi riscontrati, soluzioni adottate o qualsiasi altra informazione utile per questa attività.
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-warning">
                            <i class="ri-save-line me-1"></i>Salva Note
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal fade" id="uploadAllegatoModal" tabindex="-1" aria-labelledby="uploadAllegatoModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ url('utente/commesse/'.$commessa->id.'/attivita/'.$attivita->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="carica_allegato" value="1">
                    <div class="modal-body">
                        <div class="mb-3">
